...
std::vector<Stipple> LBGStippling::stipple(const QImage &density,
  ...
  while (notFinished(status, params)) {
    ...
    //printf("        indexMap.count %d\n",indexMap.count());
    //printf("        indexMap.size %d %d\n",indexMap.width,indexMap.height);
    ...
    if (mode == 2)
    {
    hysteresis = updateHysteresis(status.iteration, finalhysteresis, params.hysteresisDelta/10);
