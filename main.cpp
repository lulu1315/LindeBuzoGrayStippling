...
int main(int argc, char* argv[]) {
    ...
    char *initialply =               argv[15];
    char *flowfile1 =                argv[16]; //.flo file or previous frame
    char *flowfile2 =                argv[17]; // current frame
    ...
    if (argc == 1)
        ...
        if (flowmode == 0) //no opticalflow
            ...
            mystipples = m_stippling.stipple(density,params,0,initialstipples,finalhysteresis,hysteresis_strategy);
            }
        if (flowmode >= 1) 
        ...
        // calculate optical flow with deepflow
            ...
            printf("--- resetting hysteresis to  %f\n",(((hysteresis_strategy-1)*finalhysteresis)+params.hysteresis)/(hysteresis_strategy));
            ...
            float init_stipple_xpos,init_stipple_ypos,init_stipple_zpos,init_stipple_size;
            for (int i=1;i<=nvertices;i++)
                ...
                initialstipples.push_back({QVector2D(init_stipple_xpos,init_stipple_zpos), init_stipple_size, Qt::black});
                }
            inply.close();
            ...
            //performs stippling
            LBGStippling m_stippling;
            mystipples = m_stippling.stipple(density,params,2,initialstipples,finalhysteresis,hysteresis_strategy);
            //mystipples = m_stippling.stipple(density,params,3,initialstipples,finalhysteresis);
            //see warped result
            //mystipples=initialstipples;
            }
        //prepare drawings
        ...
        for (const auto &stipple : mystipples) {
            ...
            outply << stipple.pos[0] << " 0 " << stipple.pos[1] << " " << stipple.size << "\n";
            }
        painter.end();
        outply.close();
        //save result image
